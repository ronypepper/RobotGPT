...
parser.add_argument(
    "--disable_success_term",
    action=argparse.BooleanOptionalAction,
    default=False,
    help="Removes the success term of the environment. Episode successes must be manually signalled by pressing controller's 'B' button twice.",
)
...
def create_environment_config(
    ...
    success_term = None
    if not args_cli.disable_success_term:
...
def process_success_condition(env: gym.Env, success_term: object | None, success_step_count: int, manual_success: bool) -> tuple[int, bool]:
    ...
    """
    if (success_term is not None and bool(success_term.func(env, **success_term.params)[0])) or manual_success:
...
def run_simulation_loop(
    ...
    # Additional state control triggered by button presses
    ...
    prev_a_button, prev_b_button = False, False

    def control_state_by_controller_buttons():
        from isaacteleop.retargeting_engine.tensor_types import ControllerInputIndex

        nonlocal prev_a_button, prev_b_button

        # Only right controller is available via this way
        right_data = teleop_interface._session_lifecycle.last_right_controller
        if right_data is not None and not right_data.is_none:
            # A button should actually already be used in IsaacTeleopDevice, but I cannot observe its effect, so I reuse it here
            a_button = float(right_data[ControllerInputIndex.PRIMARY_CLICK]) > 0.5
            if a_button and not prev_a_button:
                toggle_recording_instance()
                pass
            prev_a_button = a_button

            b_button = float(right_data[ControllerInputIndex.SECONDARY_CLICK]) > 0.5
            if b_button and not prev_b_button:
                success_recording_instance()
    ...
    def inner_loop():
        ...
        nonlocal running_recording_instance, label_text, manual_success, manual_success_counter
            ...
            while simulation_app.is_running():
                ...
                if use_isaac_teleop:
                    ...
                    if ctrl.is_active is not None:
                        running_recording_instance = ctrl.is_active and running_recording_instance
                    if ctrl.should_reset:
                        should_reset_recording_instance = True

                    # Handle additional state control triggered by buttons
                    if use_isaac_teleop:
                        # Called after teleop_interface.advance() so controller data is fresh
                        control_state_by_controller_buttons()
                ...
                success_step_count_new, success_reset_needed = process_success_condition(
                    env, success_term, success_step_count, manual_success
                )
