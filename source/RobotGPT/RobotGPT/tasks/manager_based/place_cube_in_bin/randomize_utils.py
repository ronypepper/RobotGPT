...
#     asset_cfgs: list[SceneEntityCfg] = [SceneEntityCfg("robot")],
# ):
#     """Reset the asset root states to a random position and velocity uniformly within the given ranges, keeping a
#     minimal separation between all objects.
...
#     if num_assets == 0 or max_num_iter < 1:
#         return
#     # extract the used quantities (to enable type-hinting)
#     assets: list[RigidObject | Articulation] = [env.scene[cfg.name] for cfg in asset_cfgs]
#     # get default root states
#     root_states = torch.stack([asset.data.default_root_state[env_ids].clone() for asset in assets])

#     # poses
#     range_list = [pose_range.get(key, (0.0, 0.0)) for key in ["x", "y", "z", "roll", "pitch", "yaw"]]
#     ranges = torch.tensor(range_list, device=assets[0].device)
#     rand_samples = math_utils.sample_uniform(ranges[:, 0], ranges[:, 1], (num_assets, len(env_ids), 6), device=assets[0].device)

#     for i in range(max_num_iter):
#         positions = root_states[:, :, 0:3] + env.scene.env_origins[None, env_ids] + rand_samples[:, :, 0:3]
...
#     rand_samples = math_utils.sample_uniform(ranges[:, 0], ranges[:, 1], (len(env_ids), 6), device=assets[0].device)
