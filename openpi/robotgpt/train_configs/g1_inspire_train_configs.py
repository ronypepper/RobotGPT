...
class G1InspireInputs(transforms.DataTransformFn):
...
@dataclasses.dataclass(frozen=True)
class G1InspireOutputs(transforms.DataTransformFn):
    """
...
@dataclasses.dataclass(frozen=True)
class G1InspireDataConfig(DataConfigFactory):
    """
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[G1InspireInputs(model_type=model_config.model_type)],
            outputs=[G1InspireOutputs()],
        )
...
g1_inspire_train_configs = [
    TrainConfig(
        name="robotgpt_g1_inspire_pi05_lora",  # low_mem_finetune
        model=pi0_config.Pi0Config(
            pi05=True, paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"
        ),
        data=G1InspireDataConfig(
            repo_id=MISSING,
    ...
    TrainConfig(
        name="h100_robotgpt_g1_inspire_pi05_lora",  # low_mem_finetune on H100 GPU
        model=pi0_config.Pi0Config(
            pi05=True, paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"
        ),
        data=G1InspireDataConfig(
    ...
    TrainConfig(
        name="robotgpt_g1_inspire_pi05_base",
        model=pi0_config.Pi0Config(
            pi05=True,
        ),
        data=G1InspireDataConfig(
